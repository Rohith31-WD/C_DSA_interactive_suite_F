...
// Allocates a node holding `value`. Returns NULL on malloc failure.
...
int dcll_insertAtBeginning(dcll* list, int value)
{
    ...
    {
        // New node becomes the head; update forward and backward pointers to maintain the ring
        node->next = list->head;
        node->prev = list->tail;
        list->head->prev = node;
        list->tail->next = node;
...
int dcll_insertAtEnd(dcll* list, int value)
{
    ...
    {
        // Link after the current tail and update the ring pointers
        node->prev = list->tail;
        node->next = list->head;
        list->tail->next = node;
        list->head->prev = node;
...
int dcll_deleteByValue(dcll* list, int value)
{
    ...
    while (cur != list->head)
    {
        if (cur->data == value)
        {
            dcll_Node* prev = cur->prev;
            dcll_Node* next = cur->next;
            prev->next = next;
            next->prev = prev;
            if (cur == list->tail)
            {
...
void dcll_destroy(dcll* list)
{
    ...
    // Break the ring first so the walk has a stopping point
    list->tail->next = NULL;
    dcll_Node* cur = list->head;
    while (cur != NULL)
    {
